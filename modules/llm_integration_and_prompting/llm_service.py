...
# Import detect_language from the real path
from modules.speech_to_text_asr.backend.app.services.lang_detect import detect_language, normalize_text, transliterate_franco
...
""".strip()
    ...
    def generate(self, user_id: str, user_message: str) -> str:
        ...
        if lang == "franco-arabic":
            # Convert Franco-Arabic to proper Arabic script before sending it to the model
            normalized_input = transliterate_franco(user_message)
            # You could also run normalized_input through normalize_text(..., "arabic") if you want diacritic stripping, etc.
            ...
            # If it’s already Arabic, just normalize (strip diacritics, unify punctuation)
        ...
        # ——————————————————————————————
        # 4) CALL COHERE TEXT GENERATION
        # ——————————————————————————————
        try:
            full_prompt = f"{system_prompt}\n\nUser: {user_text_for_model}\nAssistant:"

            response = self.co.generate(
                model="command-xlarge-nightly",       # Or your chosen model
                prompt=full_prompt,
                max_tokens=150,
                temperature=0.7,
                stop_sequences=["\nUser:", "\nAssistant:"]
            ...
            print("❌ Error calling Cohere generate API:", e)
            # Return a fallback in the same detected language:
